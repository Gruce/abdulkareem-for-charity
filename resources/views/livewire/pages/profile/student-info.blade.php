<div>

    <div class="px-4 pt-4">
        <form action="#" class="flex flex-col space-y-8">

            <div>
                <h3 class="text-2xl font-semibold">المعلومات الاساسية</h3>
                <hr>
            </div>

            <div class="form-item">
                <label class="text-xl ">القسم</label>
                <input type="text"
                    class="w-full appearance-none text-gray-900 text-opacity-50 rounded shadow py-1 px-2  mr-2 focus:outline-none focus:shadow-outline focus:border-blue-200">
            </div>

            <div class="flex flex-col space-y-4 md:space-y-0 md:flex-row md:space-x-4">



                <div class="form-item w-full">
                    <label class="text-xl ">نوع الدراسة</label>
                    <input type="text" id="email"
                        class="w-full appearance-none text-gray-900  rounded shadow py-1 px-2 mr-2 focus:outline-none focus:shadow-outline focus:border-blue-200 text-opacity-25 ">
                </div>
            </div>
            <div class="flex flex-col space-y-4 md:space-y-0 md:flex-row md:space-x-4">



                <div class="form-item w-full">
                    <label class="text-xl ">المرحلة</label>
                    <input type="text" id="email"
                        class="w-full appearance-none text-gray-900  rounded shadow py-1 px-2 mr-2 focus:outline-none focus:shadow-outline focus:border-blue-200 text-opacity-25 ">
                </div>
            </div>

            <div class="flex flex-col space-y-4 md:space-y-0 md:flex-row md:space-x-4">



                <div class="form-item w-full">
                    <label class="text-xl ">الشعبة</label>
                    <input type="text" id="email"
                        class="w-full appearance-none text-gray-900  rounded shadow py-1 px-2 mr-2 focus:outline-none focus:shadow-outline focus:border-blue-200 text-opacity-25 ">
                </div>
            </div>

        </form>
    </div>

</div>